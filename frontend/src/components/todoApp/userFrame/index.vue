<template lang="">
  <div
    id="user"
    class="w-1/4 relative z-0 rounded-2xl shadow-2xl mx-4 h-full relative"
  >
    <button
      @click="logout"
      class="
        ...
        py-2
        rounded-3xl
        active:ring-4 active:ring-red-200
        ring-2 ring-gray-200
      "
    >
      <font-awesome-icon
        class="p-2 rounded-full text-5xl"
        icon="sign-out"
        fill="white"
      />
      <p class="hidden text-3xl group-hover:inline mx-8">Logout</p>
    </button>
    <button
      @click="editInfo"
      class="
    ...
    <div id="info" class="flex flex-col items-center mt-28 w-full h-2/3">
      <p v-if="firstTime" class="text-3xl text-red-400">
      ...
      <div class="flex justify-start m-4 w-11/12">
        ...
        <input
          class="text-3xl mx-8 w-1/2 outline-none truncate"
          readonly="true"
          v-model="firstName"
        />
      </div>
      ...
      <div class="flex justify-start m-4 w-11/12">
        ...
        <input
          class="text-3xl mx-8 w-3/4 outline-none truncate"
          readonly="true"
          v-model="email"
        />
      </div>
      ...
      <div class="flex justify-start m-4 w-11/12">
        ...
        <input
          class="text-3xl mx-8 outline-none truncate"
          v-model="dateOfBirth"
          readonly="true"
          type="date"
        />
    ...
    <div class="h-96 w-full py-8 opacity-0">
      <div
        id="calendar"
        class="h-80 border-2 border-black mx-auto"
        style="width:95%"
      ></div>
    </div>
  </div>
...
export default {
  ...
  created() {
    ...
    this.dateOfBirth = moment(this.$props.user.profile.dateOfBirth)
      .utc()
      .format("YYYY-MM-DD");
